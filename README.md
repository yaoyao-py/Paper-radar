...
PaperRada是一个自动化工具，用于每天获取多个数据库和期刊中包含特定关键词的最新研究文章，并通过邮件发送给用户。该工具支持多种数据源（API、RSS、网页爬取），可以根据关键词过滤文章，避免重复推送，并支持后续扩展更多期刊和关键词。
...
- 支持配置多个关键词和数据源
- 支持设置为每日定时运行（通过cron、Windows计划任务或GitHub Actions）
- 完善的日志记录和错误处理
